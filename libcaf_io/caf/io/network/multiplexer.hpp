...
public:
  ...
  /// Creates a new `dgram_servant` from a native socket handle.
  /// @threadsafe
  virtual dgram_servant_ptr new_dgram_servant(native_socket fd) = 0;

  virtual dgram_servant_ptr
  new_dgram_servant_for_endpoint(native_socket fd, ip_endpoint& ep) = 0;

  /// Create a new `dgram_servant` to contact a remote endpoint `host` and
  /// `port`.
  /// @warning Do not call from outside the multiplexer's event loop.
  virtual expected<dgram_servant_ptr>
  new_remote_udp_endpoint(const std::string& host, uint16_t port) = 0;

  /// Create a new `dgram_servant` that receives datagrams on the local `port`,
