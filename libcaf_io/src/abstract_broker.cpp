...
void abstract_broker::add_dgram_servant(dgram_servant_ptr ptr) {
  ...
  launch_servant(ptr);
  for (auto& hdl : hdls)
    add_hdl_for_dgram_servant(ptr, hdl);
}

void abstract_broker::add_hdl_for_dgram_servant(dgram_servant_ptr ptr,
                                                dgram_handle hdl) {
...
dgram_handle abstract_broker::dgram_hdl_by_port(uint16_t port) {
  for (auto& kvp : dgram_servants_)
    if (kvp.second->port(kvp.first) == port)
      return kvp.first;
  return invalid_dgram_handle;
}
