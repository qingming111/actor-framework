...
connection_state instance::handle(execution_unit* ctx,
  ...
  if (!is_handshake(hdr) && !is_heartbeat(hdr) && hdr.dest_node != this_node_) {
    ...
    if (path) {
      binary_serializer bs{ctx, visit(wr_buf_visitor{parent_}, path->hdl)};
      auto e = bs(hdr);
      if (e)
        return err();
      if (payload != nullptr)
...
void instance::handle_heartbeat(execution_unit* ctx) {
  ...
  for (auto& kvp: tbl_.direct_by_hdl_) {
    ...
    write_heartbeat(ctx, visit(wr_buf_visitor{parent_}, kvp.first), kvp.second,
                    visit(seq_num_visitor{callee_}, kvp.first));
    visit(flush_visitor{parent_}, kvp.first);
  }
}
...
void instance::write(execution_unit* ctx, const routing_table::route& r,
  ...
  write(ctx, visit(wr_buf_visitor{parent_}, r.hdl), hdr, writer);
  tbl_.flush(r);
}
...
bool instance::dispatch(execution_unit* ctx, const strong_actor_ptr& sender,
  ...
  write(ctx, visit(wr_buf_visitor{parent_}, path->hdl), hdr, &writer);
  flush(*path);
